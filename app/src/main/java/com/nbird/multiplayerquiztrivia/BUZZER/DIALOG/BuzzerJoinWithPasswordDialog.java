...
public class BuzzerJoinWithPasswordDialog {
    ...
    private void joiner(String roomCode,EditText codeEditText,AlertDialog alertDialog){
        ...
        table_user.child("BUZZER").child("ROOM").child(roomCode).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
                        ...
                        table_user.child("LeaderBoard").child(mAuth.getCurrentUser().getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                ...
                                try {
                                    ...
                                    table_user.child("BUZZER").child("PLAYERS").child(String.valueOf(room.getRoomCode())).child(mAuth.getCurrentUser().getUid()).setValue(playerInfo).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                    ...
                                    table_user.child("BUZZER").child("PLAYERS").child(String.valueOf(room.getRoomCode())).child(mAuth.getCurrentUser().getUid()).setValue(leaderBoardHolder).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            try {
                                                supportAlertDialog.dismissLoadingDialog();
